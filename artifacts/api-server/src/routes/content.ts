...
const PIPELINE_STAGES = [
  { status: "raw_idea", label: "Raw Idea" },
  ...
  { status: "repurpose", label: "Repurpose" },
...
router.get("/content", async (req, res): Promise<void> => {
  ...
  const items = conditions.length > 0
    ? await db.select().from(contentTable).where(and(...conditions)).orderBy(contentTable.createdAt)
    : await db.select().from(contentTable).orderBy(contentTable.createdAt);
...
router.post("/content", async (req, res): Promise<void> => {
  ...
  const [item] = await db
    ...
    .values({
      ...
      status: parsed.data.status ?? "raw_idea",
...
router.get("/content/pipeline", async (_req, res): Promise<void> => {
  const allItems = await db.select().from(contentTable);
  ...
  const publishedThisMonth = allItems.filter(
    (i) => i.status === "published" && i.publishDate?.startsWith(currentMonth)
  ).length;
...
router.put("/content/:id", async (req, res): Promise<void> => {
  ...
  const [item] = await db.update(contentTable).set({ ...parsed.data, updatedAt: new Date() }).where(eq(contentTable.id, params.data.id)).returning();
  if (!item) {
    res.status(404).json({ error: "Content not found" });
    return;
  }
