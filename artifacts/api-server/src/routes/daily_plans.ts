  ...
  const [existing] = await db
    ...
    .where(
      and(
        ...
        eq(dailyPlansTable.date, parsed.data.date),
      ),
    );
  if (existing) {
    const [updated] = await db
      ...
      .set({
        mits: parsed.data.mits ?? existing.mits,
        intention: parsed.data.intention ?? existing.intention,
        energyLevel: parsed.data.energyLevel ?? existing.energyLevel,
        reflection: parsed.data.reflection ?? existing.reflection,
        wins: parsed.data.wins ?? existing.wins,
        updatedAt: new Date(),
      })
      .where(
        and(
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const [plan] = await db
      ...
      .from(dailyPlansTable)
      .where(
        and(eq(dailyPlansTable.userId, userId), eq(dailyPlansTable.date, raw)),
      );
    if (!plan) {
